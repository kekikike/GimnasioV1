            ...
            <div v-if="cajaAbierta.estadoCaja === 'Abierta'" style="display:flex; flex-direction:column; gap:1rem;">
                <div style="display:flex; gap:1rem; align-items:end;">
                    <div style="flex:1;">
                        ...
                        <small v-if="montoCierre" :style="{ color: diferenciaCierre < 0.001 ? '#22c55e' : '#ef4444', fontWeight:600 }">
...
createApp({
    setup() {
        ...
        const cargarEstado = async () => {
            try {
                const res = await fetch('{{ route("recepcionista.caja.estado") }}');
        ...
        const abrirCaja = async () => {
            const res = await fetch('{{ route("recepcionista.caja.abrir") }}', {
                method: 'POST',
        ...
        const cargarMovimientos = async () => {
            try {
                const res = await fetch('{{ route("recepcionista.caja.movimientos") }}');
                const data = await res.json();
        ...
        const cargarSalidas = async () => {
            try {
                const res = await fetch('{{ route("recepcionista.caja.salidas.listar") }}');
                const data = await res.json();
                salidas.value = data.salidas || [];
        ...
        const cargarPlanes = async () => {
            try {
                const res = await fetch('{{ route("recepcionista.caja.planes") }}');
