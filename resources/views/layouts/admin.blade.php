...
<body>
    <aside class="sidebar">
        ...
        <nav class="nav">
            ...
            <a href="{{ route('admin.esquema-sueldos.index') }}" class="{{ request()->routeIs('admin.esquema-sueldos.*') ? 'active' : '' }}">
        ...
        <div class="user-info">
            <a href="{{ route('perfil') }}" style="text-decoration:none; color:inherit; display:flex; align-items:center; gap:8px;">
                ...
                <div><div class="name">{{ session('usuario')->nombre1 }} {{ session('usuario')->apellido1 }}</div>
                <div class="role">{{ session('usuario')->nombreRol ?? 'Usuario' }}</div></div>
            </a>
