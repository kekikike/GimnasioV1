    ...
    <div class="card" style="padding: 20px; margin-bottom: 20px;">
        <form @submit.prevent="guardarSocio" style="display: grid; grid-template-columns: 1fr 1fr 1fr; gap: 15px; align-items: end;">
            ...
            <div>
                ...
                <input type="number" v-model="formulario.carnetSocio" class="form-control" required placeholder="Ej. 1234567" :disabled="modoEdicion">
            </div>
            <div>
            ...
            <div style="grid-column: span 3; display: flex; gap: 10px; margin-top: 15px;">
                <button type="submit" class="btn btn-primary">@{{ modoEdicion ? '💾 Guardar Cambios' : '🔐 Registrar Socio y Generar Código' }}</button>
...
<script>
    ...
    createApp({
        setup() {
            ...
            const formBase = {
                carnetSocio: '', nombre1: '', apellido1: '', correo: '', telefono: '', 
                contrasena: '', idPlan: '', idSucursal: ''
            };
            ...
            const cargarSocios = async () => {
                try {
                    ...
                    if (data && data.success === false) {
                        alert("⚠️ Error de campos en el listado:\n" + data.message);
                        socios.value = [];
                    } else {
                        socios.value = data;
                    }
            ...
            return { socios, planes, sucursales, formulario, modoEdicion, guardarSocio, editarSocio, cancelarEdicion, congelarSocio };
        }
    }).mount('#appSocios');
</script>
@endsection